...
5. Enter the core Medusa secrets, `PHOTO_STORAGE_PROVIDER`, `BLOB_READ_WRITE_TOKEN`, and `ADMIN_GATE_SECRET` in Cloudflare by name only. Confirm no S3 provider variables are present in the selected-provider deployment environment.
...
7. Bootstrap the Medusa Admin operator. Nothing else in the pipeline creates one, so the Admin steps below are impossible until this runs. Point `DATABASE_URL` at the same staging database in the current process only:
...
```
   ...
   The seed scripts are not present in the container image, so this runs from the workstation against the staging database, exactly like step 6. The script is safe to re-run.

8. Deploy Cloudflare Medusa, record the deployment identifier, and verify `/health`, container readiness, Redis connectivity, worker heartbeat, and private storage access without logging a signed URL or token.
9. Configure the storefront values in the existing Vercel staging project, including `NEXT_PUBLIC_PHOTO_PRINT_ENABLED=true`, then deploy and record the deployment identifier.
10. Run the staging verifier against the exact Cloudflare and Vercel deployment URLs.
11. Run the required desktop and mobile browser journeys for both locales, including account draft claim, delivery, pickup, confirmation, Admin manifest access, fulfillment, and accelerated retention deletion. Reach the Admin by sending the `x-fotomax-admin-gate` header with `ADMIN_GATE_SECRET`; confirm `/app` without the header returns 404 while `/store/regions` still succeeds.
12. Record sanitized evidence, review spend telemetry, clean up synthetic test data, and rotate any temporary credential used during setup.

## Migrations

There is no automatic migration step. `wrangler deploy` ships the Worker and
container only, so step 6 must be run and confirmed **before** deploying code
that depends on a new column or table — which Phase 2B does, via eight
photo-production migrations. An earlier `predeploy` hook implied this was
automatic; it never ran, because the workspace has no `deploy` script for npm to
hook onto, and it has been removed.
