...
import { createMedusaUploadOperations, handleAbort, handleComplete, handleCreateUpload, type UploadOperations } from "./[id]/uploads/handlers"
...
const asset = { id: "phast_1", job_id: "phjob_1", display_name: "x.jpg", object_key: "photo-jobs/123e4567-e89b-12d3-a456-426614174000/originals/123e4567-e89b-12d3-a456-426614174001", expected_bytes: 12, reported_mime_type: "image/jpeg", detected_mime_type: "image/jpeg", status: "uploading" }
...
function ops(overrides: Partial<UploadOperations> = {}): UploadOperations {
  return {
    ...
    createAssetAndSession: vi.fn(async () => ({ asset, session, created: true })),
    ...
    storage: {
      startMultipartUpload: vi.fn(async () => ({ uploadId: "provider-secret" })),
      signUploadPart: vi.fn(async () => ({ url: "https://upload", expiresAt: new Date().toISOString(), requiredHeaders: {} })),
      completeMultipartUpload: vi.fn(async () => ({ etag: "etag", checksumCRC32C: "hRHAOg==" })),
      abortMultipartUpload: vi.fn(),
      headPrivateObject: vi.fn(async () => ({ bytes: 12, contentType: "image/jpeg", checksumCRC32C: "hRHAOg==" })),
...
const createBody = { filename: "x.jpg", reportedMime: "image/jpeg", bytes: 12, sourceIdempotencyKey: "idem", signatureBase64: Buffer.from(jpeg).toString("base64") }
...
describe("hardened multipart upload lifecycle", () => {
  it("rejects terminal jobs before starting provider storage", async () => {
    const operations = ops({ assertOwnedJob: vi.fn(async () => ({ id: "phjob_1", status: "cancelled" })) })
    await expect(handleCreateUpload(req(createBody), res(), operations)).rejects.toThrow("photo_upload_not_active")
  ...
  it("aborts an orphan provider upload when a concurrent create returns the winner", async () => {
    const winner = { asset: { ...asset, id: "winner" }, session: { ...session, id: "winner-session" }, created: false }
    const operations = ops({ createAssetAndSession: vi.fn(async () => winner) })
    await handleCreateUpload(req(createBody), res(), operations)
    expect(operations.storage.abortMultipartUpload).toHaveBeenCalledWith(expect.objectContaining({ uploadId: "provider-secret" }))
  })
  ...
  it("reconciles an already completed provider object without completing it twice", async () => {
    ...
    expect(operations.storage.readPrivateObjectPrefix).toHaveBeenCalledWith(asset.object_key, 12)
  ...
  it("completes a missing object then validates the actual stored signature", async () => {
    ...
    ;(storage.headPrivateObject as ReturnType<typeof vi.fn>)
      ...
      .mockResolvedValueOnce({ bytes: 12, contentType: "image/jpeg", checksumCRC32C: "hRHAOg==" })
  ...
  it("deletes invalid stored bytes and records a stable failed state", async () => {
    const storage = { ...ops().storage, readPrivateObjectPrefix: vi.fn(async () => Uint8Array.from([1, 2, 3])) }
    const operations = ops({ storage })
    await expect(handleComplete(req({ parts }, { id: "phjob_1", sessionId: "phups_1" }), res(), operations)).rejects.toThrow("photo_upload_metadata_mismatch")
    expect(storage.deletePrivateObjects).toHaveBeenCalledWith([asset.object_key])
  ...
  it("keeps the active session recoverable on transient storage inspection errors", async () => {
    const storage = { ...ops().storage, readPrivateObjectPrefix: vi.fn(async () => { throw new PhotoStorageError("photo_storage_provider_error") }) }
    const operations = ops({ storage })
    await expect(handleComplete(req({ parts }, { id: "phjob_1", sessionId: "phups_1" }), res(), operations)).rejects.toThrow("photo_storage_provider_error")
  ...
  it("preserves a valid completed object when the database commit fails", async () => {
    const operations = ops({
      completeSession: vi.fn(async () => { throw new Error("database unavailable") }),
    })

    await expect(handleComplete(req({ parts }, { id: "phjob_1", sessionId: "phups_1" }), res(), operations))
  ...
  it("resumes cleanup for an already aborted session", async () => {
    const operations = ops({ findOwnedSession: vi.fn(async () => ({ asset: { ...asset, status: "failed" }, session: { ...session, status: "aborted" } })) })
    await handleAbort(req({}, { id: "phjob_1", sessionId: "phups_1" }), res(), operations)
    expect(operations.abortSession).not.toHaveBeenCalled()
    expect(operations.storage.abortMultipartUpload).toHaveBeenCalled()
    expect(operations.storage.deletePrivateObjects).toHaveBeenCalledWith([asset.object_key])
  })
  it("claims the abort state before touching provider storage", async () => {
    const order: string[] = []
    const operations = ops({
      abortSession: vi.fn(async () => { order.push("db"); return { ...asset, status: "failed" } }),
      storage: { ...ops().storage, abortMultipartUpload: vi.fn(async () => { order.push("provider") }) },
    })
    await handleAbort(req({}, { id: "phjob_1", sessionId: "phups_1" }), res(), operations)
...
describe("serializable upload operations", () => {
  function medusaOperations(service: Record<string, any>) {
    ...
    return createMedusaUploadOperations({
      ...
      scope: { resolve: (name: string) => name.includes("storage") ? storage : service },
    })
  }
  ...
  it("enforces limits inside the serializable shared context", async () => {
    ...
    const service = {
      ...
      listPhotoAssets: vi.fn(async () => Array.from({ length: 500 }, () => ({ status: "uploading", expected_bytes: 1 }))),
      createPhotoAssets: vi.fn(),
    }
    const operations = medusaOperations(service)
    await expect(operations.createAssetAndSession({
      jobId: "phjob_1", displayName: "x.jpg", objectKey: asset.object_key,
      reportedMime: "image/jpeg", detectedMime: "image/jpeg", expectedBytes: 12,
      idempotencyKey: "new", providerUploadId: "upload", expiresAt: new Date(),
    })).rejects.toThrow("photo_asset_limit_exceeded")
    expect(service.retrievePhotoJob).toHaveBeenCalledWith("phjob_1", undefined, context)
    expect(service.listPhotoAssets).toHaveBeenCalledWith({ job_id: "phjob_1" }, {}, context)
  ...
  it("uses one context and conditional active/uploading selectors", async () => {
    ...
    expect(service.listPhotoUploadSessions).toHaveBeenCalledWith({ id: session.id }, {}, context)
  ...
  it("rejects terminal overwrite before conditional updates", async () => {
    const service = {
      withPhotoJobTransaction: vi.fn(async (callback) => callback({ transactionManager: {} })),
      listPhotoUploadSessions: vi.fn(async () => [{ ...session, status: "completed" }]),
      retrievePhotoAsset: vi.fn(async () => ({ ...asset, status: "uploaded" })),
      updatePhotoUploadSessions: vi.fn(),
      updatePhotoAssets: vi.fn(),
    }
    const operations = medusaOperations(service)
    await expect(operations.abortSession(asset, session)).rejects.toThrow("photo_upload_not_active")
