...
export async function DELETE(req: any, res: any): Promise<void> {
  ...
  const jobId = req.params?.id?.trim(); const assetId = req.params?.assetId?.trim()
  ...
  if (asset.status === "deleted") { res.json({ asset: { id: asset.id, status: "deleted" } }); return }

  const sessions = await service.listPhotoUploadSessions({ asset_id: asset.id })
  for (const session of sessions) {
    if (session.status === "active" && session.provider_upload_id) {
      await storage.abortMultipartUpload({ key: asset.object_key, uploadId: session.provider_upload_id }).catch(() => undefined)
    }
  }
  await storage.deletePrivateObjects([asset.object_key])

  const deleted = await service.withPhotoJobTransaction(async (context: any) => {
    ...
    for (const session of active) await service.updatePhotoUploadSessions({ selector: { id: session.id, status: "active" }, data: { status: "aborted", aborted_at: new Date() } }, context)
  ...
  res.json({ asset: { id: deleted.id, status: deleted.status } })
}
