...
medusaIntegrationTestRunner({ moduleName: "fotomax-photo-upload", cwd: appRoot, env, testSuite: (options) => {
  ...
  describe("private photo upload journey", () => {
    it("creates a guest job, uploads privately, reloads, and conceals it cross-owner", async () => {
      ...
      const signed = await options.api.post(`/store/photo-jobs/${job.id}/uploads/${upload.sessionId}/parts`, { partNumber: 1, checksumCRC32C: "hRHAOg==" }, { headers }); const part = record(signed.data.part)
      ...
      await options.api.post(`/store/photo-jobs/${job.id}/uploads/${upload.sessionId}/complete`, { parts: [{ partNumber: 1, etag, checksumCRC32C: "hRHAOg==" }] }, { headers })
