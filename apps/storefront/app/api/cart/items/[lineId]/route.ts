...
import { CartError, createCartAdapter, parseQuantityInput } from "../../../../../src/lib/medusa/cart"
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    quantity = parseQuantityInput(body.quantity)
  } catch (error) {
  ...
  try {
    ...
    return NextResponse.json({ cart: await adapter.updateLine(cartId, lineId, quantity) })
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const { lineId } = await params
    const adapter = createCartAdapter(await createStoreSdk())
    return NextResponse.json({ cart: await adapter.removeLine(cartId, lineId) })
  } catch (error) {
    if (errorStatus(error) === 404) {
      return expiredResponse()
    }
