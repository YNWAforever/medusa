...
export async function proxyAssetDelete(request: NextRequest, path: string): Promise<NextResponse> {
  if (!isAllowedPhotoMutationOrigin(request.headers, configuredStorefrontOrigin(request.nextUrl.origin))) return response({ error: { code: "photo_job_origin_forbidden" } }, 403)
  try { const env = getStorefrontEnv(); const customerToken = request.cookies.get(CUSTOMER_TOKEN_COOKIE)?.value; const candidate = request.cookies.get(PHOTO_GUEST_COOKIE)?.value; const guestSecret = validPhotoGuestSecret(candidate) ? candidate : undefined; const upstream = await fetch(new URL(path, env.backendUrl), { method: "DELETE", headers: photoRequestHeaders({ customerToken, guestSecret, extra: { "x-publishable-api-key": env.publishableKey } }), cache: "no-store" }); const payload = await upstream.json().catch(() => ({ error: { code: "photo_job_unavailable" } })); return response(payload, upstream.status) } catch { return response({ error: { code: "photo_job_unavailable" } }, 502) }
}
